...
```{r}
p2 <- p2 + geom_point(aes(size = year, 
                          alpha = 0.25, 
                          color = "red")) #within bracket could be anything, its not detecting color red it could be "redddddddd"
# this is wrong. Why? alpha cannot be within aes() since aes() links variables to scales.
# This says I want transparency to be a value
# but what you want is aes() to a transparency of 0.25, its not an actual value
# This would be correct: aes(alpha = count) then trasnparency (variable) is linked to a scale
...
```{r}
p2 <- p2 + geom_point(aes(color = continent, 
                          size = year),
                          alpha = 0.5)
p2
```
...
# Make a second plot
p3 <- ggplot(gapminder, aes(x = log10(gdpPercap),
                            y = lifeExp)) +
...
# ggplot2 is the package and ggplot is the function within the package
