...
```{r}
gapminder %>% 
  #arrange(country, year) this would arrange by country and within by year
    group_by(country) %>% 
...
Calculate by element the result, work on each line independently
...
Take a bunch of number and returns 1 number
...
Take a bunch of number and returns equal bunch of number
    ...
    # windows function:
    # spply: Works to vectorize a list, returns a list
    # apply: matrix
...
- The population, in billions. (aggregate)
- The lagged gdpPercap (window)
...
gapminder %>%
  group_by(country) %>%
  summarize(sum = sum(as.numeric(pop))) %>%
  mutate(pop_growth = pop[year==1972]-pop[year==1962])
# mine doesnt work^
# mutate will not work without summarize, because it assigns same number of rows
