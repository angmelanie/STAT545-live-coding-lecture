...
This is a copy of [cm013 on stats website](https://github.com/STAT545-UBC/STAT545-UBC.github.io/blob/master/cm013-notes_and_exercises.Rmd)
...
Note: use tribble, tells you what the type of and class is as well.
...
you can change direction from 1 to -1 to flip the colours 
```{r}
...
p <- gapminder %>%
  dplyr::select(year, continent, lifeExp) %>%
  group_by(continent, year) %>%
  summarize(lifeexpyear = sum(lifeExp)) %>%
  ggplot(aes(x=year, y = lifeexpyear)) +
  geom_histogram(aes(x=year, y=lifeexpyear), stat = "identity") +
  facet_wrap(~ continent) 

p <- gapminder %>%
  dplyr::select(year, continent, lifeExp) %>%
  ggplot(aes(x=year, y = lifeExp)) +
  geom_histogram(aes(x=year, y=lifeExp, fill=lifeExp), stat="identity") +
  facet_wrap(~ continent) 
...
ggplot(gapminder, aes(lifeExp)) +
  facet_wrap(~continent) +
  geom_histogram(aes(y=lifeExp, fill=continent), stat = "identity", binwidth = 5, bins = 5)
?geom_histogram

  scale_fill_brewer(palette = "Blues")
p + scale_fill_manual(values = brewer.pal(n=5, "Dark2"))
```
