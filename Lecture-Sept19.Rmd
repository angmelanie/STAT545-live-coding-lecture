...
```{r}
typeof("5")
# because surrounded by quotation
```
...
```{r}
petal.width <- iris$Petal.Width # assign to variable
hist(petal.width) # histogram
frequency(petal.width)
table(petal.width)
```
...
```{r}
filter(gapminder, country=="Canada"| country == "Algeria" , year >= 1960 & year <=1969)
# filter(gapminder, country=="Canada"| country == "Algeria" , year == c(1960:1969)) # wrong
...
```{r}
mutate(gapminder,
  cc = paste("country", "continent", sep = ","))

gapminder$cc <- mutate(gapminder,
  cc = paste("country", "continent", sep = ","))
