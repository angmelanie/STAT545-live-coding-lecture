...
output: 
    html_document:
        toc: true
---
[Link to class website](https://stat545.com/cm007-notes_and_exercises.html)
...
# se is standard error aka error bars
...
ggplot(gapminder, aes(x=year, y=lifeExp, colour=continent)) +
  geom_smooth(method="lm", se=FALSE)
# it does not need the bottom layer in order to perform additional operations
```


### Facetting
# basically group by either 1 dimension or 2 dimensions
...
ggplot(gapminder, aes(x=year, y=lifeExp)) +
  ...
  geom_smooth(se=FALSE, method = "lm", span = .5)
...
```{r, fig.width=8} 
# applies all figure within size 8 (wider than usual) within code chunk
vc2 <- gapminder %>% 
    mutate(size=c("small", "large")[(pop>7000000) + 1]) %>% 
    ggplot(aes(gdpPercap, lifeExp)) +
    facet_grid(size ~ continent) # x scale = continent, y scale = size
vc2 + geom_point()

# ggplot works with + operator and not %>% 
```
...
** you need to include this within the beginning code chunk**

Example: life expectancy over time for each country.

```{r}
## Without the `group` specification, incorrect:
...
# how to display countries as diff colours?
ggplot(gapminder, aes(year, lifeExp, group = country)) + 
...
**Connect dots in the order they appear in the data frame**
...
```{r}
# country from above question but group by and colour by country
ggplot(gapminder, aes(x=year, y=pop, group=country, colour = country)) +
  geom_line(alpha = 0.2) +
  geom_point()

ggplot(gapminder, aes(x=year, y=pop, group=country, colour = gdpPercap)) +
  geom_line(alpha = 0.2) +
  geom_point()

ggplot(gapminder, aes(x=year, y=pop, group=country)) +
  geom_line(alpha = 0.2, aes(colour=country)) +
  geom_point()
